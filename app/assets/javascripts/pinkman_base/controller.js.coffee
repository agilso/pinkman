...
class window.PinkmanController extends window.PinkmanObject
  ...
  # construct a controller from user definition
  make: ->
    if @maker? and typeof @maker == 'function'
      @maker(this)
  ...
  action: (id,eventName,callback='') ->
    a = new PinkmanAction(id: id, eventName: eventName, call: callback)
    a.listen() if a.callback? and typeof a.callback == 'function'
    return(a)
...
class window.PinkmanAction extends window.PinkmanObject
  ...
  # expected functionality: c.action('edit','click').sameAs('anotherPage','edit')
  sameAs: (controller,action) ->
...
Pinkman.controllers.def = (id,maker) ->
  if id? and maker? and $('#'+id).length > 0
    c = new PinkmanController(id: id, maker: maker)
